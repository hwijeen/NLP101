class Node():
    def __init__(self, word, constituent, origin=None, origin_idx=None):
        self.word = word
...
class CKYParser():
    ...
    def merge(self, word, node_list1, node_list2):
        ...
        for node1 in node_list1:
            for node2 in node_list2:
                for constituent1 in node1.constituent:
                    for constituent2 in node2.constituent:
                        if self.grammar_check(constituent1, constituent2):
                            for merged_constituent, origin in self.binary_grammar.items():
                                if (constituent1, constituent2) in origin:
                                    ...
                                    merged.append(Node(word, merged_constituent, (node1, node2), ))
        return merged

    def dfs(self, table, node, visited, output):
        if node not in visited:
            visited.append(node)
            output += "(" + str(node.constituent[0]) + " "    # FIXME : which constituent?
            if type(node.word)==str: output += node.word
            if node.origin is None:    # leaf node
                output += ")"
                return visited, output
            for child in node.origin:
                visited, output = self.dfs(table, child, visited, output)
            output += ")"
        return visited, output
    ...
    def print_tree(self, input_len):
        ...
        for head in possible_heads:
            visited, output = self.dfs(self.table, head, [], "")
            print(output)
    ...
    def parse(self, input):
        ...
        for i in range(len(input)):
            for j in range(i + 1):
                if i-j==i: continue
                #print("buildind %d %d" % (i-j,i))
                for k in range(i-j, i):
                    #print("from %d %d & %d %d" % (i-j,k,k+1,i))
                    self.table[i-j][i] += self.merge(input[i-j:i+1], self.table[i-j][k],
                                                     self.table[k + 1][i])   # merge(Node1, Node2)
        self.table
