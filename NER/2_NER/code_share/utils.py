...
class Config():
    ...
    #진짜는 Train_CoNLL.txt
    filename_dev = "../data/dev_CoNLL.txt"
    filename_test = "../data/test_CoNLL.txt"
    filename_train = "../data/train_CoNLL.txt"
...
class data_read(object):
    # iterable. for x in data_read의 instance 형태로 사용 가능!
    ...
    def __len__(self):
        ...
        if self.length is None:
            self.length = 0
            for _ in self:    # QUESTION: iterating over self?
                self.length += 1
...
def data_build():
    ...
    processing_word = get_processing_word()    # NOTE: does nothing when no dictionary given!
    ...
    vocab_words = set()
    vocab_tags = set()
    # QUESTION: test data의 단어도 포함시키네?
    for dataset in [train, dev, test]:
        for words, tags in dataset:
            vocab_words.update(words)    # NOTE: words is a list!
            vocab_tags.update(tags)
    print("- done. {} tokens".format(len(vocab_words)))

    # pretrained에 있는 단어/형태소의 vocab 만들기
    ...
    vocab_for_embed = load_vocab(config.filename_words)    # is a dictionary
    ...
    with open(config.filename_embedding) as f:
        for line in f:
            ...
            if word in vocab_for_embed:    # pretrain단어 중에 우리 사전에 있는 것만 가져오기
